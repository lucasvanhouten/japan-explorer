...
function checks(stops, legs, t, opt) {
  ...
  Object.entries(byLoc).forEach(([loc, visits]) => {
    ...
    else if (total < range[0]) N(`${nm}: ${total} nights is under the ideal ${range[0]}–${range[1]}; it has room to absorb spare nights.`);
    if (total > range[1] + 1) V(`${nm}: ${total} nights is more than one over the top of its ideal ${range[0]}–${range[1]} — past the +1 ceiling unless they asked.`);
  ...
  for (let i = 2; i < stops.length; i++) {
    ...
    const thru = L && L.source === "table" && stops.slice(0, i - 1).find((s) => isBaseKind(s.kind) && s.loc !== stops[i - 1].loc && s.loc !== stops[i].loc && new RegExp("\\b" + short(s.loc) + "\\b").test(L.text || ""));
...
/* a leg cell reads to five minutes, exactly as the spine timeline prints it, so `plan` and `spine` never
 * disagree about the same journey; the Totals line keeps the raw sum (2026-09-13) */
const legBody = (l) => (l.estimated ? "to confirm" : `${hm5(l.h)} · ${l.x ? chg(l.x) : "direct"} · ${l.mode}${l.source === "table" ? "" : ` · ${l.source}`}`);
...
function printPlan(P, opt) {
  ...
  /* one rounding only (2026-09-13): the total reads to five minutes, exactly as the leg rows do, and no
   * arithmetic string is printed beside it — two roundings of the same journey never agree */
  L.push("", `**Totals:** ${hoursTxt5(t)} of travel · ${perNightTxt(t)} · ${t.changes} changes · ${t.checkins} check-ins · ${t.innNights} inn nights / ${t.cityNights} city nights${t.roomOnlyNights ? ` (${t.roomOnlyNights} room only)` : ""} · ${t.oneNighters} one-nighters.${capNote}`);
...
const DRAWS = ["food", "temples", "nature", "onsen", "craft", "city", "pop", "art", "snow", "coast"];
...
const legWord = (l) => (l.estimated ? "to confirm" : `${hm5(l.h)} ${modeWord(l.mode)}`);
...
const yesNoDecision = (y, owner) => ({ kind: y.kind || "stop", key: y.key, question: y.question, default: y.default == null ? 1 : y.default,
  on_at: y.on_at, owner, options: [
    ...
    { label: "No", desc: y.no || NO_DESC, stops: [] }] });
...
function choicesOf(sp, sets, total, repeat, apIn) {
  const ch = {};
  for (const d of decisionsOf(sp)) {
    ...
    if (apIn && d.key === "start") { const j = d.options.findIndex((o) => o.in === apIn); if (j >= 0) i = j; }
...
function assembleOnce(sp, choices, forceReverse) {
  ...
  cl.forEach((c, i) => {
    const at = atFor[i] || [];
    at.filter((x) => x.at === "before").forEach((x) => x.o.stops.forEach((l) => seq.push(l)));
    seq.push(c);
    at.filter((x) => x.at !== "before").forEach((x) => { x.o.stops.forEach((l) => seq.push(l)); if (x.at === "split") seq.push(c); });
...
function whyNot(sp, d, o, choices, opt) {
  ...
  let struct = null;
  if (o.needCity && !o.needCity.some((c) => cl.includes(c))) struct = `${label(o.needCity[0])} is not on this route`;
...
function assembleLocs(sp, choices, sets, opt) {
  ...
  for (let pass = 0; pass < 6; pass++) {
    ...
    for (const d of decisionsOf(sp)) {
      ...
      if (!why) continue;
      if (sets && sets[d.key] != null) throw new Error(`${d.key}=${o.label} is not available here: ${why}`);
  ...
  return { locs, cities, reversed, choices, unavailable };
}
...
function fitTotal(stops, N, repeat) {
  ...
  while (total() < N && guard++ < 200) {
    ...
    for (const [s] of bases.slice().reverse()) { if (total() >= N) break; const top = rangeOf(s.loc, { repeat })[1];
      if (atLoc(s.loc) === top) { s.nights++; moved = true; steps.push(`${label(s.loc)} takes one night over its usual ${rangeOf(s.loc, { repeat }).join("–")} (${s.nights - 1} → ${s.nights})`); } }
    if (!moved) break;
  }
...
function assembleSpine(sp, optIn) {
  ...
  const asked = choicesOf(sp, sets, opt.total, opt.repeat, opt.in);
  ...
  let { locs, cities, reversed, choices, unavailable } = assembleLocs(sp, asked, sets, opt);
  ...
  P.spine = sp; P.choices = choices; P.cities = cities; P.reversed = reversed; P.steps = steps; P.band = bandOf(sp, locs); P.unavailable = unavailable; P.fitNote = fitNote;
...
function spineHead(sp, band, opt) {
  const own = sp.band.join("–"), here = band ? band.join("–") : own;
  const io = (opt && (opt.in || opt.out)) ? `${opt.in ? apLabel(opt.in) : "—"} → ${opt.out ? apLabel(opt.out) : "—"}` : sp.io;
...
const optCell = (d, i, chosen, rev) => d.options.map((o, j) => { const w = optWord(o, rev); return `${j === chosen ? `**${j + 1} ${w.label}**` : `${j + 1} ${w.label}`} — ${w.desc}`; }).join(" · ");
...
function groupsOf(sp, choices, cities, reversed) {
  ...
  /* anything the assembly cannot reach at all still exists as a decision; it is listed last and, having one real
   * option left, never printed */
  const rest = ds.filter((d) => !groups.some((g) => g.items.includes(d)));
  if (rest.length) groups.push({ where: "Not on this route", kind: "off", items: rest });
  return groups;
}
...
function decisionsTable(sp, choices, unavailable, cities, reversed) {
  const L = ["| Where | Decision | Options (chosen in bold) |", "|---|---|---|"];
  let n = 0;
  for (const g of groupsOf(sp, choices, cities, reversed)) {
    let first = true;
    for (const d of g.items) {
      const na = (unavailable || []).filter((u) => u.key === d.key).map((u) => `${u.label}: not offered here — ${u.why}`);
      if (d.options.length - na.length < 2) continue; n++;
      ...
      L.push(`| ${first ? `**${g.where}**` : ""} | ${n} · ${KIND_LABEL[d.kind] || d.kind.toUpperCase()} · ${d.question}${on} \`${d.key}\` | ${optCell(d, 0, choices[d.key], reversed)}${na.length ? ` · _${na.join("; ")}_` : ""} |`);
      first = false; }
  }
  return L.join("\n");
}
  ...
  if (P.spine && P.choices) {
    ...
    for (const d of ds) {
      ...
      for (const j of order) {
        ...
        out.push({ P: Q, what: `${d.key}: ${d.options[cur].label} → ${d.options[j].label}` });
      }
    }
  }
...
function cmdSpines(opt) {
  ...
  rows.forEach((r) => { r.noTicket = ticket && r.P.flights >= TICKET_FLIGHT_MAX;
    ...
    if (r.noTicket && !opt.repeat) { try { const R = assembleSpine(r.sp, { total: r.at, in: opt.in, out: opt.out, reverse: true }); if (R.flights < TICKET_FLIGHT_MAX) { r.P = R; r.noTicket = false; r.reversed = true; } } catch (e) { /* stays as it was */ } } });
  ...
  rows.forEach(({ sp, P, hit, band, out, at, noTicket }) => { const t = P.totals;
    const forCell = `${sp.for || "—"}${out ? ` Runs ${band[0]}–${band[1]} nights, so it is shown at ${at}.` : ""}${rows.find((x) => x.sp === sp).reversed ? " Runs the other way round for this ticket." : ""}`;
...
function cmdSpine(ref, opt) {
  ...
  const before = opt.before ? buildPlan(parseStops(opt.before), defined({ in: opt.in, out: opt.out, repeat: opt.repeat })) : assembleSpine(sp, base);
  ...
  const moved = changed && (opt.before || stopString(before.stops) !== stopString(after.stops) || decisionsOf(sp).some((d) => before.choices && before.choices[d.key] !== after.choices[d.key]));
  ...
  const L = [spineHead(sp, after.band, opt), "", lineOf(sp, chosenOpts), ""];
...
function cmdStays(q, opt) {
  ...
  const G = stayGroup(q);
  const loc = G ? G.locs[0] : resolveLoc(q), p = placeOf(loc);
  ...
  const held = (inns.length - Math.min(inns.length, cap)) + Math.max(0, shownHotels.length - nFitH);
  ...
  const L = [`*${where} (${G ? `${G.locs.length} towns, one table` : p.kind + (p.unlisted ? ", no card line" : "")}): ${shownTxt}${budget ? ` within a ${budget} budget` : ""}${opt.bath ? ", in-room bath only" : ""}, best first${reachTxt}${held ? ` — ${held} more on the shortlist here (--all for every one)` : ""}. The Why column is the kit's write-up, your source for a Why in your own words; the band is per night for two, dinner and breakfast included for an inn, room only for a hotel.*`, ""];
  ...
  const innRow = (i, town) => `| ${innStay(i)} | ${G ? `${town} | ` : ""}${i.band} | ${i.bath} | ${i.writeup} | ${i.map ? `[map](${i.map}) · ` : ""}[full write-up](${i.url}) |`;
  ...
  const gate = G ? GROUP_GATE[norm(q).replace(/ /g, "-")] : null;
  const townOf = (i) => { const t = i.area || label(i.loc || loc); if (!gate || !i.loc) return t; const l = leg(gate, i.loc); return l && !l.estimated && l.h != null ? `${t} · from ${label(gate)} ${hm(l.h)}${l.x ? `, ${l.x} change${l.x > 1 ? "s" : ""}` : ", direct"}` : `${t} · from ${label(gate)}: to confirm`; };
  ...
  if (!rowsI.length && !rowsH.length) {
    ...
    if (near) { L.push("", `The nearest place the kit covers is ${label(near.loc)}, ${Math.round(near.km)} km away:`, "", "| Stay | Band | Bath | Why | Links |", "|---|---|---|---|---|");
      near.inns.slice(0, 3).forEach((i) => L.push(`| ${innStay(i)} | ${i.band} | ${i.bath} | ${i.writeup} | ${i.map ? `[map](${i.map}) · ` : ""}[full write-up](${i.url}) |`)); }
  }
...
function main(argv) {
  ...
  for (let i = 0; i < argv.length; i++) { const a = argv[i];
    ...
    else if (a === "--budget") opt.budget = String(argv[++i] || ""); else if (a === "--draws") opt.draws.push(...String(argv[++i] || "").split(","));
    else pos.push(a); }
