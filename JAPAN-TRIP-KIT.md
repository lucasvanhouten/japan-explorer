...
**This stage orients; it does not price.** No table of routes, no travel figures, no night counts beyond a card's ideal range. The spines, the engine and every number wait for Stage 3.
...
- **`spines:` names the routes that carry the place** — Stage 3's line; here it only tells you which cards chain together. A place on no spine is said so in a clause (Okinawa, or a deeper Tōhoku than the spines reach).
...
A **spine** is an order of major cities, with every decision hanging off a city or off the leg between two — which cities and which way round, a city's own extra night, the ryokan and stop towns on a leg, how the trip ends. Nine spines cover the country, and the places they warmed to in Stage 2 sit on two or three of them. **Lead with the comparison, not a list**, in this shape:

1. **A paragraph per route**, in your own words: what the trip feels like day to day, what each stop gives them, what it trades away, how many times it moves. The route's name in bold; no id.
2. **A side-by-side table**, one column per route: `| | <Route> | <Route> |`, rows **Stops** · **Nights by city** · **Inn nights** · **The extra** (what this route has that the other lacks) · **Travel** (from the engine: hours, per night, check-ins) · then **one row per draw they gave** ("Cocktails and bars", "Hot springs") saying what each route does for it.
3. **"Pick A if… Pick B if…"** — one paragraph tying each route to the words in their profile, and which you would take.
