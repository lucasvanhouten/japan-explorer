...
1. **A paragraph per route**, in your own words, in this order, the name in bold and no id: **the major cities** in trip order, a clause each on what it gives; **the ryokan options** — every one the run's *Ryokan nights* line prints, what the place is and where it sits (out and back from Tokyo, on the way to Kyoto) — and **the town stops** the same way, from its *Town stops* line; then **the usual nights in each major city as a range**, from its *Usual nights* line. No defaults, travel figures or last day here: those arrive with the walk. **Every place and range comes from one engine run per route at the profile's nights and ticket** — `spine <name> --total N --in X --out Y` — and **the walk opens on exactly that assembly**.
2. **The differences**, one short paragraph across the routes: what each has that the others lack, how many times each moves, which draw each serves. A table only if asked. The menu ranks first the routes that open where they land and close where they fly home.
